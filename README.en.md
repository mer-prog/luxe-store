...
- **Compensating transactions** that release reserved stock if Stripe session creation fails (no stock leaks)
...
- **Unit tests** — the payment and inventory core is covered by 21 Vitest tests
...
| **Validation** | Zod | Runtime schema validation on auth, product and review inputs |
| **Testing** | Vitest | Unit tests for the payment/inventory core (Stripe & Prisma mocked) |
...
- **Product Catalog** — Filterable by category, sortable, with keyword search (case-insensitive partial match)
...
Cart → POST /api/checkout
            │
            ├─ Stock validation
            ├─ Optimistic stock lock (conditional decrement inside $transaction)
            ├─ Order created (status: PENDING)
            └─ Stripe Checkout Session created (30 min expiry)
                    │
                    ├─ Creation fails → compensating transaction:
                    │     stock released + order CANCELLED / FAILED
                    ▼
 ...
 • CONFIRMED    • Stock kept until
   / PAID         session expiry
 • Save address   (customer can retry)
 • Clear cart
 • Send email
     │
     ▼
...
**Edge cases handled:**
- Stripe session creation failure → compensating transaction releases reserved stock and cancels the order (no stock leaks)
- Session expiration (30 min) → stock restored, order cancelled
- Failed payment attempt → order resolved via Checkout Session lookup, payment status set to FAILED; stock is kept until the session expires so the customer can retry
- Email failure → logged, does not block order completion
- Concurrent checkout → optimistic stock locking prevents overselling
- Duplicate webhook delivery → idempotency check skips already-paid orders
...
| **Input Validation** | Zod schemas on auth, product and review inputs |

---

## Testing

The payment/inventory core is covered by **21 Vitest unit tests**. Stripe and Prisma are fully mocked — no real API keys or database required.

| Target | What is verified |
|:-------|:-----------------|
| `POST /api/checkout` (`tests/checkout.test.ts`) | Auth check, upfront stock validation, optimistic locking (conditional decrement / abort on conflict), tax line item, **compensating transaction on Stripe session-creation failure (stock released + order cancelled)** |
| Webhook signatures (`tests/webhook-signature.test.ts`) | 400 on missing/invalid signature, raw body + secret passed to `constructEvent`, unhandled events acknowledged |
| Webhook handlers (`tests/webhook-handlers.test.ts`) | `completed` idempotency, cart clearing, shipping address persistence; `expired` stock restore; `payment_failed` session lookup and no downgrade of PAID orders |
...
│   ├── order-number.ts           # Order number generation (LUXE-YYYYMMDD-NNN)
...
tests/                            # Vitest unit tests (Stripe / Prisma mocked)
├── checkout.test.ts              # Stock reservation/release + compensation
├── webhook-signature.test.ts     # Webhook signature verification
...
| **Compensating transactions** | If Stripe session creation fails after stock is reserved, stock is released and the order cancelled — no order is left unreachable by webhooks |
| **Prisma over raw SQL** | Type-safe queries with generated types; schema-first migrations |
...
| **Locale-linked currency formatting** | Display currency switches with language (¥ ↔ $); payment logic (cent integers) remains unchanged |
